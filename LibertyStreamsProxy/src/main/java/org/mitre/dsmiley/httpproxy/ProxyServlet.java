...
public class ProxyServlet extends HttpServlet {
  ...
  public void init() throws ServletException {
	  System.out.println("init() : here we are...");
  ...
  protected void initTarget() throws ServletException {
    ...
    //test it's valid
    System.out.println("initTarget@targetUri:" + targetUri);
    try {
    ...
    System.out.println("initTarget@targetHost:" + targetHost);    
  }
   ...
   * 
   * todo * need to pickup the correct IP and port for the mapping, values from the VCAP. 
   */
  protected void copyRequestHeader(HttpServletRequest servletRequest, HttpRequest proxyRequest,
    ...
    while (headers.hasMoreElements()) {//sometimes more than one value
      ...
      if (headerName.equalsIgnoreCase(HttpHeaders.HOST)) {
        ...
        if(host == null) {
            log("HttpHost returned null");	
        }
        log("HttpHost:" + host.toString());
        headerValue = host.getHostName();
        if (host.getPort() != -1)
          headerValue += ":"+host.getPort();
        log("headerValue including port : " + headerValue);
